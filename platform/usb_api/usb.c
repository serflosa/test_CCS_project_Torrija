...
|  RSTO         2009/11/18   updated USB_connectionState()                    |
+----------------------------------------------------------------------------*/
/*----------------------------------------------------------------------------+
| Include files                                                               |
+----------------------------------------------------------------------------*/
...
static uint8_t* pbIEP0Buffer;      // A buffer pointer to input end point 0
...
void USB_InitSerialStringDescriptor(void)
{
    ...
    Get_TLV_info(TLV_DIERECORD, &bBytes, (uint16_t*)&pbSerNum); //The die record used for serial number
...
//WATCHOUT: DEPENDANT OF CRYSTAL FREQ (USB - PLL Set for 32.0 MHz input clock) 
    USBPLLDIVB = USB_XT_FREQ;               // Settings desired frequency. Set at descriptors.h
    if (USB_PLL_XT == 2)
    {
        //Error: USBPLLCTL = UPCS0 + UPFDEN + UPLLEN;// Select XT2 as Ref / Select PLL for USB / Discrim. on, enable PLL
...
/*
    ...
    tInputEndPointDescriptorBlock[EDB(HID_INEP_ADDR)].bEPBBAX  = (uint8_t)(((IEP1_X_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    tInputEndPointDescriptorBlock[EDB(HID_INEP_ADDR)].bEPBBAY  = (uint8_t)(((IEP1_Y_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    ...
    tOutputEndPointDescriptorBlock[EDB(HID_OUTEP_ADDR)].bEPBBAX  = (uint8_t)(((OEP1_X_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    tOutputEndPointDescriptorBlock[EDB(HID_OUTEP_ADDR)].bEPBBAY  = (uint8_t)(((OEP1_Y_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
...
*/
    ...
    tInputEndPointDescriptorBlock[EDB(CDC_INTEP_ADDR)].bEPBBAX  = (uint8_t)(((IEP2_X_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    tInputEndPointDescriptorBlock[EDB(CDC_INTEP_ADDR)].bEPBBAY  = (uint8_t)(((IEP2_Y_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    ...
    tInputEndPointDescriptorBlock[EDB(CDC_INEP_ADDR)].bEPCNF   = EPCNF_USBIE       // use Interrupt for this EP
                                                | EPCNF_UBME      // enable this EP
                                                | EPCNF_DBUF;     // use double buffering (X and Y buffers)
    tInputEndPointDescriptorBlock[EDB(CDC_INEP_ADDR)].bEPBBAX  = (uint8_t)(((IEP3_X_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    tInputEndPointDescriptorBlock[EDB(CDC_INEP_ADDR)].bEPBBAY  = (uint8_t)(((IEP3_Y_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    ...
    tOutputEndPointDescriptorBlock[EDB(CDC_OUTEP_ADDR)].bEPCNF   = EPCNF_USBIE      // use Interrupt for this EP
                                                 | EPCNF_UBME     // enable this EP
                                                 | EPCNF_DBUF;    // use double buffering (X and Y buffers)
    tOutputEndPointDescriptorBlock[EDB(CDC_OUTEP_ADDR)].bEPBBAX  = (uint8_t)(((OEP3_X_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
    tOutputEndPointDescriptorBlock[EDB(CDC_OUTEP_ADDR)].bEPBBAY  = (uint8_t)(((OEP3_Y_BUFFER_ADDRESS - START_OF_USB_BUFFER) >> 3) & 0x00ff);
...
void usbSendDataPacketOnEP0(uint8_t* pbBuffer)
{
    uint16_t wTemp;
...
void usbReceiveNextPacketOnOEP0(void)
{
    ...
    bByte = tEndPoint0DescriptorBlock.bOEPBCNT & EPBCNT_uint8_tCNT_MASK;
...
uint8_t usbDecodeAndProcessUsbRequest(void)
{
    ...
    // tDEVICE_REQUEST_COMPARE  -->  member of structure
    // void(*pUsbFunction)(void) --> function pointer in the structure we want to call 
    lAddrOfFunction = *((uint32_t*)((pbUsbRequestList) + 10));
    ...
    if (((tpF)lAddrOfFunction == &usbSetAddress) && (USBFUNADR != 0))
    { bWakeUp = TRUE; }// bWakeUp = USB_handleEnumCompleteEvent();
